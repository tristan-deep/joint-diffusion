...
class ActNorm(nn.Module):
    def __init__(self, channels, device):
        ...
        self.logs = torch.nn.Parameter(
            torch.zeros(size, dtype=torch.float, device=device, requires_grad=True)
        )
...
if __name__ == "__main__":
    ...
    for i in range(5000):
        opt.zero_grad()
        x = torch.tensor(
            np.random.normal(5, 10, size), dtype=torch.float, device=device
        )
        ...
        if i % 10 == 0:
            print(
                "\r loss=%0.3f | logdet=%0.3f | b=%0.3f | s=%0.3f | rev=%0.3f "
                % (
                    loss,
                    -logdet2.mean().item(),
                    actnorm.b.mean().item(),
                    np.exp(actnorm.logs.mean().item()),
                    rev_loss,
                ),
                end="\r",
            )
