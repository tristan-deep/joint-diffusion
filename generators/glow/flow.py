...
class Flow(nn.Module):
    def __init__(
        self, channels, coupling, device, coupling_bias, nn_init_last_zeros=False
    ):
        super(Flow, self).__init__()
        self.actnorm = ActNorm(channels, device)
        self.coupling = CouplingLayer(
            channels, coupling, coupling_bias, device, nn_init_last_zeros
        )
...
if __name__ == "__main__":
    ...
    for i in range(5000):
        ...
        print(
            "\rmse=%0.3f | mu=%0.3f | std=%0.3f | rloss=%0.3f | logdet=%0.3f"
