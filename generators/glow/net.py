...
class NN(nn.Module):
    def __init__(self, channels_in, channels_out, device, init_last_zeros=False):
        ...
        self.conv1 = nn.Conv2d(
            channels_in, 512, kernel_size=(3, 3), stride=1, padding=1, bias=True
        )
        self.actnorm1 = ActNorm(512, device)

        self.conv2 = nn.Conv2d(
            512, 512, kernel_size=(1, 1), stride=1, padding=0, bias=True
        )
        self.actnorm2 = ActNorm(512, device)

        self.conv3 = nn.Conv2d(
            512, channels_out, kernel_size=(3, 3), stride=1, padding=1, bias=True
        )
...
if __name__ == "__main__":
    ...
    for i in range(5000):
        ...
        if i % 10 == 0:
            print(
                "\r loss=%0.3f| mu=%0.3f | std=%0.3f" % (loss.item(), mu, std), end="\r"
            )
