...
class CouplingLayer(nn.Module):
    def __init__(
        self, channels, coupling, coupling_bias, device, nn_init_last_zeros=False
    ):
        ...
        if self.coupling == "affine":
            self.net = NN(
                ...
                device=device,
                init_last_zeros=nn_init_last_zeros,
            )
    ...
    def forward(self, x, logdet=None, reverse=False):
        ...
        if not reverse:
            ...
            if self.coupling == "affine":
                ...
                s = torch.sigmoid(s + 2.0) + self.coupling_bias
                # t       = torch.tanh(t)
                ya = s * xa + t
...
if __name__ == "__main__":
    ...
    coupling = CouplingLayer(
        channels=64, coupling="affine", device=device, nn_init_last_zeros=False
    )
